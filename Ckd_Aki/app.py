...
with open('Ckd_Aki/kidney_disease_voting_model.pkl', 'rb') as model_file:
    loaded_voting_clf = pickle.load(model_file)

with open('Ckd_Aki/label_encoder.pkl', 'rb') as le_file:
...
def predict():
    try:
        ...
        data = request.json
        if not data or 'features' not in data or 'age' not in data:
            return jsonify({"error": "Invalid input format. Please provide 'features' and 'age' in JSON format."})
        ...
        # Generate prevention report using Gemma API
